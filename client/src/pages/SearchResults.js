  ...
  return (
    <Container maxW="container.xl" py={8}>
      ...
      {results?.hostel?.length > 0 && (
        <Box mb={8}>
          <Heading size="md" mb={4}>Nhà trọ ({results.hostel.length})</Heading>
          <SimpleGrid columns={{ base: 1, md: 2, lg: 3 }} spacing={6}>
            {results.hostel.map(hostel => (
              ...
              >
                ...
                />
                <Box p={4}>
                  <Heading size="md" mb={2}>{hostel.name}</Heading>
      ...
      {results?.rooms?.length > 0 && (
        <Box>
          <Heading size="md" mb={4}>Phòng trống ({results.rooms.length})</Heading>
          <SimpleGrid columns={{ base: 1, md: 2, lg: 3 }} spacing={6}>
            {results.rooms.map(room => (
              ...
              >
                ...
                />
                <Box p={4}>
                  <Heading size="md" mb={2}>{room.roomName}</Heading>
                  ...
                  <Text color="gray.600">
                    {room.hostel.address}, {room.hostel.district}, {room.hostel.city}
                  </Text>
                </Box>
              </Box>
            ))}
