...
const TenantRoomList = () => {
  ...
  return (
    <Box p={4}>
      <Center mb={4}>
        <Heading fontSize="2xl" fontWeight="bold">Thông tin phòng</Heading>
      </Center>
      <SimpleGrid columns={{ base: 1, sm: 2, md: 3, lg: 4 }} spacing={6}>
        {rooms.map((room) => (
          ...
          >
            <Image src={room.image} alt={`Phòng ${room.id}`} w="100%" h="200px" objectFit="cover" />
            <VStack p={4} align="stretch" spacing={2} flex={1}>
              <Box>
                <Flex alignItems="flex-start">
                  <Text fontWeight="bold" color="black" width="70px" flexShrink={0}>Địa chỉ:</Text>
                  <Text color="gray.600" fontSize="sm">{room.address}</Text>
                </Flex>
              </Box>
              <Flex justifyContent="space-between" alignItems="center">
                <Text fontWeight="bold" color="black">Tình trạng:</Text>
                <Tag 
                  colorScheme={room.status.toLowerCase().includes("còn trống") ? "green" : "orange"}
              ...
              <Flex justifyContent="space-between">
                <Text fontWeight="bold" color="black">Giá:</Text>
              ...
              {selectedRoom && (
                <Flex direction={{ base: "column", md: "row" }} gap={6}>
                  ...
                  <VStack align="stretch" spacing={4} w={{ base: "100%", md: "50%" }}>
                    <Box>
                      <Text fontWeight="bold" color="black">Địa chỉ:</Text>
                    ...
                    <Box>
                      <Text fontWeight="bold" color="black">Tình trạng:</Text>
                      <Tag 
                        colorScheme={selectedRoom.status.toLowerCase().includes("còn trống") ? "green" : "orange"}
                        size="md"
                        fontSize="md"
                      >
                        {selectedRoom.status}
                    ...
                    <Box>
                      <Text fontWeight="bold" color="black">Tiện ích:</Text>
                      <List spacing={2}>
